...
> Version 0.1.9 · macOS 13 Ventura or later · Apple Silicon
...
Packages through `v0.1.9` are ad-hoc signed, so each version has a different macOS privacy identity. Moving to the first Developer ID signed release requires granting Screen Recording once more. Later releases signed by the same identity retain that permission.
